...
// eslint-disable-next-line no-var -- must be var for cross-script re-injection guard (const throws SyntaxError on re-injection)
var _COLOR_NAMES_VERSION = 2;
if (typeof globalThis._colorNamesVersion !== 'undefined' && globalThis._colorNamesVersion === _COLOR_NAMES_VERSION) {
    // Already loaded with same version
} else {
    globalThis._colorNamesVersion = _COLOR_NAMES_VERSION;
    ...
    const ColorNames = {
        ...
        nearest(hex) {
            ...
            let h = hex.replace('#', '').substring(0, 6).toLowerCase();
            if (h.length === 3) h = h[0] + h[0] + h[1] + h[1] + h[2] + h[2];
            else if (h.length === 4) h = h[0] + h[0] + h[1] + h[1] + h[2] + h[2]; // #rgba → #rrggbb
            if (h.length !== 6) return { name: 'Unknown', hex: '#000000', distance: Infinity };
