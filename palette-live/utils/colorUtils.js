...
const _COLOR_UTILS_VERSION = 3;
if (window._colorUtilsVersion === _COLOR_UTILS_VERSION) {
  // Already loaded with same version — skip re-declaration
} else {
  window._colorUtilsVersion = _COLOR_UTILS_VERSION;
  const ColorUtils = {
    ...
    rgbToHex8: (input) => {
      ...
      const s = input.trim();
      if (!s) return '#000000';
      if (s.startsWith('#')) return ColorUtils._normalizeHex8(s);
      if (s.startsWith('rgb')) return ColorUtils._rgbStringToHex8(s);
      // For other formats, fall back to DOM resolution which loses alpha
      // but at least returns a valid color
      return ColorUtils.rgbToHex(s);
    },
    ...
    parseAlpha: (input) => {
      ...
      if (s.startsWith('rgba') || s.startsWith('hsla')) {
        ...
        if (nums && nums.length >= 4) return Number(nums[3]);
      }
      return 1;
    },
