...
if (window._heatmapVersion === _HEATMAP_VERSION) {
    ...
    const Heatmap = {
        ...
        toggle: (active) => {
            ...
            if (active) {
                const id = ++Heatmap._showId;
                // show() is async; fire-and-forget is intentional here
                Heatmap.show(id).catch((err) => console.warn('PaletteLive Heatmap: show() failed', err));
            } else {
                Heatmap.hide();
            }
        },
        ...
        show: async (id) => {
            ...
            if (!Heatmap.isActive || id !== Heatmap._showId) return;
            ...
            for (let i = 0; i < elements.length; i += BATCH_SIZE) {
                ...
                // If heatmap was toggled off or a newer show() started, bail out
                if (!Heatmap.isActive || id !== Heatmap._showId) return;
            ...
            // Bail out if deactivated or superseded during element scanning
            if (!Heatmap.isActive || id !== Heatmap._showId) return;
            ...
            if (window.__PALETTE_LIVE_DEV__) {
                console.log(
                    `PaletteLive Heatmap: ${candidates.length} elements highlighted across ${colorFrequency.size} colors`
                );
            }
        },
